...
test.describe('Landing Page', () => {
  ...
  test('loads landing page with all caesars', async ({ page }) => {
    ...
    // Verify all 12 caesars are displayed (cards with imperial-numeral class)
    const caesarCards = page.locator('.imperial-numeral');
    await expect(caesarCards).toHaveCount(12);

    // Check specific caesars are present
    await expect(page.locator('a[href="/julius/"]')).toBeVisible();
    await expect(page.locator('a[href="/augustus/"]')).toBeVisible();
    await expect(page.locator('a[href="/nero/"]')).toBeVisible();
  });
  ...
  test('caesar cards have proper structure and content', async ({ page }) => {
    ...
    const juliusCard = page.locator('a[href="/julius/"]');
  ...
  test('navigation to caesar pages works', async ({ page }) => {
    ...
    // Click on Julius
    await page.click('a[href="/julius/"]');
    await page.waitForLoadState('networkidle');

    // Should navigate to Julius page
    await expect(page).toHaveURL(/\/julius\/$/);
